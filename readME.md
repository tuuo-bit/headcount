...
**Python GUI and Excel for Attendance Management**

Taking attendance made fair, time efficient and easier by recognizing WiFi SSID's in range rather than physical counting. This program is only intended to be used a tool and by no means is a foolproof method for its shortcomings.

## Misconception

**Only the following two lines from the script require Admin privileges ( and why it could be considered a virus) :**
...
Turns ON and OFF the WiFi interface to refresh and scan for new networks in range.
...
* Excel files are only modified in the same folder as the python file "gui.py"
